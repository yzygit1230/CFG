...
class DiceLossWithMask(nn.Module):
    ...
    def forward(self, inputs, target, mask=None, weight=None, softmax=False, sigmoid=False, multi=False):
        ...
        # assert inputs.min() >= 0 and target.max() < 2, "标签值超出范围"
...
def l_correlation_cos_mean(model1, model2, linear_paras1):  # fix model2
    ...
    print(f"Length of linear_paras1: {len(linear_paras1)}")
    ...
    for name, parameters in model1.named_parameters():
        if 'conv' in name and 'weight' in name:
            if len(parameters.shape) == 4:  
                ...
                if count >= len(linear_paras1):
                    print(f"Index out of range: count={count}, length of linear_paras1={len(linear_paras1)}")
                    continue 
    ...
    total_loss = total_loss / count if count > 0 else torch.tensor(0.0, requires_grad=True).cuda()
    return total_loss
