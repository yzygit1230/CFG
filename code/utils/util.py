...
def draw_contour(
    ...
    dilated = cv2.dilate(binary, np.ones((5, 5), dtype='uint8'), iterations=1)  # 这里调整轮廓粗细
...
def draw_contour_and_save(img, pred, mask, save_path='resedge/1.png'):
    if len(pred.shape) == 2:
    ...
    predi = pred.squeeze(0).cpu().numpy().astype(np.uint8) 
    maski = mask.squeeze(0).cpu().numpy().astype(np.uint8) 

    edges_predi = cv2.Canny(predi * 255, 100, 200) 
    edges_maski = cv2.Canny(maski * 255, 100, 200) 
    kernel = np.ones((3,3), np.uint8)  
    edges_predi = cv2.dilate(edges_predi, kernel, iterations=1)
    edges_maski = cv2.dilate(edges_maski, kernel, iterations=1)

    img = img.astype(np.uint8)  

    img[edges_predi != 0] = [0, 255, 0]  
    img[edges_maski != 0] = [0, 0, 255]  

    cv2.imwrite(save_path, img)

def draw_binary_mask_and_save(img, pred, save_path='resbinary/1.png'):
    if len(pred.shape) == 2:
        pred = pred.unsqueeze(0)
    if len(img.shape) == 2:
        img = img.unsqueeze(0)
